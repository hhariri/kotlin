...
fun generators(): List<GenericFunction> {
    ...
    templates add f("merge(other: Map<K, V2>, transform: (K, V?, V2?) -> R)") {
        only(Maps)
        doc {
            """
            Returns a map built from elements of both collections with same keys using provided *transform*.
            """
        }
        typeParam("V")
        typeParam("V2")
        ...
        body {
            ...
            val map = hashMapOf<K, R>()
            for (key in keys) {
                map.put(key, transform(key, get(key), other.get(key)))
            }
            return map
            """
        }
